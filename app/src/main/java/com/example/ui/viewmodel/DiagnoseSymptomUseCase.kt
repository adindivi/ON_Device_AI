...
class DiagnoseSymptomUseCase(
    ...
    suspend fun executeStream(queryDtc: String, querySymptom: String, isQwenEnabled: Boolean = true): Flow<DiagnosisResult> {
        ...
        if (!isQwenEnabled) {
            val allMatches = backendEngine.ragSearcher.search(fullQuery, topK = 10)
            ...
            return flow {
                emit(DiagnosisResult(history = fastHistory, matches = allMatches))
            }
        }
